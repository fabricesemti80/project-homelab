...
out_dir="${repo_root}/infra/docker/secrets/omni"
...
chmod 600 "${out_dir}/admin-password"

block_file="$(mktemp)"
trap 'rm -f "$block_file"' EXIT
{
  printf '# Generated by infra/docker/scripts/render-omni-envrc.sh. Do not edit this block by hand.\n'
...
start="# >>> omni generated secrets >>>"
end="# <<< omni generated secrets <<<"
tmp="$(mktemp)"
if [ -f "$envrc" ]; then
  awk -v start="$start" -v end="$end" '$0 == start {skip = 1; next} $0 == end {skip = 0; next} skip != 1 {print}' "$envrc" >"$tmp"
fi
{
  cat "$tmp"
  printf '\n%s\n' "$start"
